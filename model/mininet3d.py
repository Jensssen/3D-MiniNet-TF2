...
class MiniNet3D:
    ...
    def build(self, train_logical: bool, verbose: bool):
        ...
        #################################input_image = tf.reshape(points, (-1, self.input_dim[0], self.num_class[1], 5))
        # input_image: (?, 64, 512, 5):  (b, h, w, 5)

        inputs = tf.keras.layers.Input(shape=self.input_dim, name="input_tensor")

        point_groups = tf.image.extract_patches(inputs, ksizes=[1, 4, 4, 1], strides=[1, 4, 4, 1],
        ...
        x = self.conv_layer(input_final, (1, 1), 24, train_logical, name="point-based_local_1")
        ...
        x_branch = self.downsample(inputs, n_filters_out=192, is_training=train_logical)
